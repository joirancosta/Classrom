...
app.get("/courses", async (request: Request, response: Response) => {
  ...
  const courses = await knex("courses").select("id", "name").orderBy("name", "desc")
...
app.get("/courses/:id/modules", async (request: Request, response: Response) => {
  ...
  .select("courses.id AS course_id", "course_modules.id AS module_id", "course_modules.name AS module", "courses.name AS course")
